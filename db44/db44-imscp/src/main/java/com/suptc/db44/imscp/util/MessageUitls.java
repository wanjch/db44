...
public class MessageUitls {
	...
	private final static String delimiter = ImscpConfig.get("delimiter");
	private final static String data_delimiter = ImscpConfig.get("data_delimiter");
	...
	public static String genMessage(String functionCode, String platformCode, String[] data) {
		...
		msg.append(ImscpConfig.get("begin")).append(functionCode).append(delimiter).append(platformCode).append(delimiter)
				.append(body.length()).append(delimiter).append(body).append(ImscpConfig.get("end"));
		return msg.toString();
	}
